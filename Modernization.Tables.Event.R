...
#remove those from habs that only mismatch in project code
habs<-anti_join(habs,event,by=c('EVENT_LMAS_LOCATION_HISTORY_ID','EVENT_LMAS_SAMPLE_DATE','EVENT_LMAS_DATA_PROVIDER','EVENT_LMAS_SAMPLE_TIME'))
...
#check, there shouldn't be multiple samples per location/date
check2<-event %>% 
  group_by(EVENT_LMAS_LOCATION_HISTORY_ID,EVENT_LMAS_SAMPLE_DATE,EVENT_LMAS_SAMPLE_TIME,EVENT_LMAS_DATA_PROVIDER) %>% 
  mutate(n=n()) %>% 
  ungroup() %>% 
  filter(n>1) %>% 
  distinct()
